	      ...
	      <li class="has-dropdown hide-for-medium-up">
	      	<a href="#">Crawl Actions</a>
		      	<ul class="button-group dropdown even-2">
		      		<li class="divider"></li>
			      	<li><div><button class="button" type='submit' name='action' value='build' <#if job.hasApplicationContext> disabled='disabled' title='build job'</#if> >build</button></div></li>
			      	<li class="divider"></li>
			      	<li><div><button class="button" type='submit' name='action' value='launch' 
					<#if !job.availableActions?seq_contains("launch") || job.isProfile>
					 disabled='disabled'
					</#if>
					 >launch</button></div></li>
			      	<li class="divider"></li>
			        <li><div><button class="button" <#if !job.availableActions?seq_contains("pause")> disabled</#if> type='submit' name='action' value='pause'>pause</button></div></li>
			      	<li class="divider"></li>
					<li><div><button class="button" <#if !job.availableActions?seq_contains("unpause")> disabled</#if> type='submit' name='action' value='unpause'>unpause</button></div></li>
			      	<li class="divider"></li>
		        	<li><div><button class="button" <#if !job.isRunning> disabled</#if>  type='submit' name='action' value='checkpoint'>checkpoint</button></div></li>
			      	<li class="divider"></li>
			        <li><div><button class="button" <#if !job.isRunning> disabled</#if> type='submit' name='action' value='terminate'>terminate</button></div></li>
		      		<li class="divider"></li>
			        <li><div><button class="button" type='submit' name='action' value='teardown' <#if !job.hasApplicationContext> disabled='disabled' title='no instance'</#if>>teardown</button></div></li>
			        <li class="divider"></li>
		      	</ul>
	  	  </li>
	      <li class="divider"></li>
